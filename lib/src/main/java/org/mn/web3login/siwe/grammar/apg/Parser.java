...
/**
 * The Parser class is used to construct a parser for a specific grammar. It is
 * initialized with a generated Grammar class object and optional UDT and rule
 * callback functions. Optionally, the Parser class can be configured to
 * generate an AST, parsing statistics, and/or a trace of the syntax tree nodes
...
public class Parser {
    ...
    public Parser(Grammar grammar) {
        initInstance(grammar);
    }

    /**
     * Enables or disables the generation of an Abstract Syntax Tree (AST). The
     * Ast class has no public constructor and this function is the only way to
     * generate an instance.
     *
     * @param enable if true, an AST will be generated, if false, not. By
    ...
    public Ast enableAst(boolean enable) {
        if (enable) {
            if (sysData.ast == null) {
                sysData.ast = new Ast(this, grammar);
            }
    ...
    /**
     * Enables or disables the generation of a Trace. Trace has no public
     * constructor and this function is the only way to generate an instance.
     *
     * @param enable if true, an trace will be generated, if false, not. By
     * default enable is false.
    ...
    public Trace enableTrace(boolean enable) {
        if (enable) {
            if (sysData.trace == null) {
                sysData.trace = new Trace(this, grammar);
            }
        } else {
            sysData.trace = null;
        }
    ...
    /**
     * Enables or disables the generation of a parsing statistics. Statistics
     * has no public constructor and this function is the only way to generate
     * an instance.
     *
     * @param enable if true, statistics will be generated, if false, not. By
     * default enable is false.
    ...
    public Statistics enableStatistics(boolean enable) {
        if (enable) {
            if (sysData.stats == null) {
                sysData.stats = new Statistics(grammar);
            }
        } else {
            sysData.stats = null;
        }
    ...
    /**
     * Sets the start rule. By default <code>start = 0</code> which identifies
     * the first rule in the grammar definition.
     *
     * @param start the identifier for the desired start rule.
     * @see GeneratorGrammar.RuleNames
     * @throws IllegalArgumentException thrown if the <code>start</code>
    ...
    /**
     * Sets the input string to be parsed. A call to this function is required
    ...
    public void setInputString(char[] input) {
        ...
        if (sysData.ast != null) {
            sysData.ast.setInputString(input);
        }
    }
     ...
     * @param callback an instance of a user-written extension of the
     * RuleCallback class.
     * @see GeneratorGrammar.RuleNames
     */
    ...
    /**
     * Called to provision the callback functions with a user-defined data
     * class. <code>null</code> by default.
     *
     * @param data any object of the user's choice.
     */
    public void setMyData(Object data) {
        sysData.callbackData.myData = data;
    }

    /**
     * Call this function to parser the input string. A call to
     * {@link #setInputString(String) setInputString} is required prior to
     * calling this function.
     *
     * @return a Result class with information about the parser results.
     * @throws Exception thrown if any callback function returns an illegal
     * value. Illegal values are phrase lengths longer than the length of the
     * remaining input string or a zero (empty) length by a UDT designated as
     * non-empty. (UDTs having names beginning with "<code>u_</code>" are
     * designated as non-empty. UDTs having names beginning with
     * "<code>e_</code>" are designated as empty.)
     * @see Result
     */
    public Result parse() throws Exception {
        ...
        if (sysData.ast != null) {
            sysData.ast.beginAst();
        }
        if (sysData.trace != null) {
            sysData.trace.beginTrace(sysData.inputString);
        }
        ...
        if (sysData.trace != null) {
            sysData.trace.endTrace(sysData.inputString);
        }
    ...
    public class CallbackData {
        ...
        /**
         * Optional user-defined data for use by all rule and UDT callback
         * functions.
         *
    ...
    public static class RuleCallback {
        ...
        /**
         * Called by the parser just prior to parsing the syntax tree branch
         * below this rule node. This callback function can do nothing, perform
         * semantic actions or can function fully as a terminal node, skipping
         * the parser's processing of the branch below entirely.
         *
         ...
         * indicating to the parser that this is a terminal node, or
         * <code>-1</code> to continue normally. If acting as a terminal node
         * the parser will skip normal processing of the branch below and the
         * phrase length must be in the range <code>0 &lt;= L &lt;=(n-1)</code>,
         * where <code>n</code> is the remaining number of characters in the
         * input string.
         * @throws Exception thrown if the callback function returns an illegal
         * value. An illegal value would be a phrase length
         * <code>L &gt;= n</code>, for example.
         */
        ...
        /**
         * Called by the parser just following the parsing of the syntax tree
         * branch below this rule node. This callback function can do nothing,
         * perform semantic actions or override the parser's result, acting as a
         * terminal node.
         *
         * @param offset offset into the input string of the first character of
         ...
         * @return the return value can be a phrase length, <code>L</code>,
         * indicating to the parser that this is a terminal node overriding the
         * parser's result, or <code>-1</code> to continue normally. If acting
         * as a terminal node, the phrase length must be in the range
         * <code>0 &lt;= L &lt;=(n-1)</code>, where <code>n</code> is the
         * remaining number of characters in the input string.
         * @throws Exception thrown if the callback function returns an illegal
         * value. An illegal value would be a phrase length
         * <code>L &gt;= n</code>, for example.
         */
    ...
    public static abstract class UdtCallback {
         ...
         *
         * @param parser the instance of the Parser to attach this callback
         * function to.
         */
         ...
         * The UDT function.
         *
         * @param offset offset into the input string to the first character of
         * the parsed phrase.
         * @return the matched phrase length, <code>L</code>, or <code>-1</code>
         * if no phrase was matched. The phrase length must be in the range
         * <code>0 &lt;= L &lt;=(n-1)</code>, where <code>n</code> is the
         * remaining number of characters in the input string.
         * @throws Exception thrown if the callback function returns an illegal
         * value. An illegal value would be a phrase length
         * <code>L &gt;= n</code>, or Illegal values are phrase lengths longer
         * than the length of the remaining input string or a zero (empty)
         * length by a UDT designated as non-empty. (UDTs having names beginning
         * with "<code>u_</code>" are designated as non-empty. UDTs having names
         * beginning with "<code>e_</code>" are designated as empty.)
         */
    ...
    public class Result {
         ...
         * and the entire input string was consumed. If <code>false</code> the
         * parse was unsuccessful. This could mean that the state was
         * <code>false</code> or that the entire input string was not consumed
         * or both.
         */
        public boolean success() {
            return state && (matchedPhraseLength == inputString.length);
        }

        /**
         * Called to get the final state of the parser. Note that if the matched
         * phrase is not the entire input string the parse will not succeed even
         * though the state is <code>true</code>
         *
         * @return if <code>true</code> a phrase was successfully matched, if
         * <code>false</code> no phrase was successfully matched. Note that if
         * <code>true</code> the phrase matched may not be the entire input
         * string.
         */
         ...
         *
         * @return length of the matched phrase. This will be zero if
         * <code>getState()</code> returns <code>false</code>. If if
         * <code>getState()</code> returns <code>true</code> the length returned
         * may or may not be the full length of the input string.
         */
        public int getMatchedPhraseLength() {
            return matchedPhraseLength;
        }

        /**
         * Called to get the maximum matched phrase length.
         *
         * @return the maximum phrase length matched during the parse. May be
         * <code>0</code> even if <code>getState()</code> returns
         * <code>false</code>. Can be useful in locating the point at which the
         * Parser failed.
         */
        public int getMaxMatchedPhraseLength() {
            return maxMatchedPhraseLength;
        }
        ...
        public void displayResult(PrintStream out) {
            if (success()) {
                out.print("   parser result: success\n");
            } else {
    ...
    /**
     * <i>NOTE: This function should only be called from UdtCallback or
     * RuleCallback callback functions. Results will be unpredictable if called
     * otherwise.</i><p>
     * Parses the requested rule as if it were a child node of the calling
     * function. A call to this function alters the grammar being parsed and
     * should be used with care.
     *
     * @param id grammar id of the rule to parse. Must be in the range
     * <code>0 - (rule count-1)</code>.
     * @param offset offset of the first character in the input string of the
     * phrase to match.
     * @return the length of the matched phrase if the parse is successful.
     * <code>-1</code> otherwise. Phrase lengths must in the range
     * <code>0 - (n-1)</code>, where <code>n</code> is the remaining string
     * length.
     * @throws IllegalArgumentException thrown if id is out of range (see
     * above).
     * @throws Exception thrown if the callback function returns an illegal
     * value. An illegal value would be a phrase length <code>L &gt;= n</code>
     * or a zero (empty) length by a UDT designated as non-empty. (UDTs having
     * names beginning with "<code>u_</code>" are designated as non-empty. UDTs
     * having names beginning with "<code>e_</code>" are designated as empty.)
     */
     ...
     * RuleCallback callback functions. Results will be unpredictable if called
     * otherwise.</i><p>
     * Parses the requested UDT as if it were a child node of the calling
     * function. A call to this function alters the grammar being parsed and
     * should be used with care.
     *
     * @param id grammar id of the UDT to parse. Must be in the range
     * <code>0 - (UDT count-1)</code>.
     * @param offset offset of the first character in the input string of the
     * phrase to match.
     * @return the length of the matched phrase if the parse is successful.
     * <code>-1</code> otherwise. Phrase lengths must in the range
     ...
     * RuleCallback callback functions. Results will be unpredictable if called
     * otherwise.</i><p>
     * Adds a rule node to the AST. May alter the grammar the AST is
     * representing and should only be used with care.
     *
     ...
     * @param length length of the matched phrase. Must not be larger than the
     * remaining input string length.
     * @param match is not used.
     * @throws IllegalArgumentException thrown if the id is out of range or the
     * @throws Exception thrown if the callback function returns an illegal
     * value. An illegal value would be a phrase length <code>L &gt;= n</code>,
     * where <code>n</code> is the remaining length of the input string, or a
     * zero (empty) length by a UDT designated as non-empty. (UDTs having names
     * beginning with "<code>u_</code>" are designated as non-empty. UDTs having
     * names beginning with "<code>e_</code>" are designated as empty.)
     */
     ...
     * RuleCallback callback functions. Results will be unpredictable if called
     * otherwise.</i><p>
     * Adds a UDT node to the AST. May alter the grammar the AST is representing
