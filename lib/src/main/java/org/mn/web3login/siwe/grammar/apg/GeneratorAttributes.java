...
//		buf.append(ruleAttrs[0][0].legend());
    ...
    private boolean isCatCyclic(Attrs[] children) {
        ...
        for (Attrs a : children) {
            if (!a.cyclic) {
                ret = false;
                break;
            }	// if any child is NOT cyclic, CAT is not cyclic (ie. S = SS is cyclic)
        }
        return ret;
    }
    ...
    private boolean isCatLeft(Attrs[] children) {
        ...
        for (Attrs a : children) {
            ...
            if (!a.empty) {
                ret = false;
                break;
            }	// else if this child is not empty, CAT is not left
    ...
    private boolean isCatRight(Attrs[] children) {
        ...
        for (int i = children.length - 1; i >= 0; i -= 1) {
            if (children[i].right) {
                ret = true;
                break;
            }	// if right-most, non-empty child is right, CAT is right
            if (!children[i].empty) {
                ret = false;
                break;
            }	// else if this child is not empty, CAT is not right
            // else this child is empty - keep looking
        }
        return ret;
    }
    ...
    private boolean isNonEmptyOnly(Attrs a) {
        boolean ret = false;
        if (a.empty && !(a.left || a.right || a.nested || a.cyclic || a.finite)) {
            ret = true;
        }
        return ret;
    }
    ...
    private boolean isRecursive(Attrs a) {
        boolean ret = false;
        if (a.left || a.right || a.nested || a.cyclic) {
            ret = true;
        }
        return ret;
    }
