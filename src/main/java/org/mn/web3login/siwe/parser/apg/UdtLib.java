...
/**
 * This is the parent class for a library of User-Defined Terminals (UDTs). UDTs
 * are provided for some simple and repetitive phrases that are common to most
 * languages designed to be in human-readable, ASCII text.
 */
public class UdtLib {
    ...
    // public API - the UDTs
    /**
     * UDT for recognizing alpha-numeric phrases. Alpha-numeric phrases begin
     * with an alphabetic character <code>a-z</code> or <code>A-Z</code>.
     * Characters after the first may be alphabetic or numeric,
     * <code>0-9</code>.
     */
    public static class Alphanum extends Parser.UdtCallback {
        ...
        /**
         * Constructor defining one other character which is allowed in addition
         * to alpha-numeric after the first.
         *
         * @param p the Parser object the UDT is attached to.
    ...
    /**
     * UDT for recognizing a string of any printing characters. That is, ASCII
     * characters <code>32-126</code>.
     */
    public static class Any extends Parser.UdtCallback {
        ...
        /**
         * Constructor to include/exclude spaces (<code>x20</code>) from the set
         * of allowed characters.
         *
         * @param p the Parser object the UDT is attached to.
         * @param spaceAllowed if true, spaces are included, if false, spaces
         * are excluded.
         */
    ...
    /**
     * UDT for recognizing the C-language comments. These are strings of all
     * UTF-8 characters which begin with the phrase "<code>/*</code>" and end
     * with "<code>*&#47;</code>". Nesting of beginning and ending phrases not
     * allowed.
     */
    public static class CComment extends Parser.UdtCallback {
        ...
        public int callback(int offset) {
            ...
            if ((i + 1) < inlen && c == SLASH && callbackData.inputString[ii] == STAR) {
                ...
                for (; ii < inlen; i++, ii++) {
                    c = callbackData.inputString[i];
                    if (c == STAR && callbackData.inputString[ii] == SLASH) {
                        len += 2;
                        break;
                    }
    ...
    /**
     * UDT for many common comment formats. Comments are defined by one or two
     * leading characters and the comment will include all following characters
     * up to but not including the next line end character (<code>x0A</code> or
     * <code>x0D</code>)
     */
    public static class Comment extends Parser.UdtCallback {
         ...
         * @param begin the first character which defines the beginning of the
         * comment.
         */
        ...
        public int callback(int offset) {
            ...
            if (!isDoubleChar) {
                if (i < inlen && callbackData.inputString[i] == begin1) {
                    ...
                    for (; i < inlen; i++) {
                        c = callbackData.inputString[i];
                        if ((c >= PRINTING_BEG && c <= PRINTING_END) || c == HTAB) {
                            len++;
                        } else {
                            break;
                        }
    ...
    /**
     * UDT for the C++ language comment. This is an extension of the Comment
     * class which defines the beginning pair of characters to be "//".
     */
    ...
    /**
     * UDT for the ABNF comment. This is an extension of the Comment class which
     * defines the leading comment character to be the semi-colon,
     * "<code>;</code>".
     */
     ...
     * <p>
     * This is a string of at least <code>min</code> and not more than
     * <code>max</code> decimal digits, where
     * <code>0 &lt;= min &lt;= max</code>. A decimal digit is <code>0-9</code>.
     */
    public static class DecNum extends Parser.UdtCallback {
         ...
         * The default constructor.
         * <p>
         * The default values are <code>min = 0, max = infinite</code>. Note
         * that <code>min = 0</code>, indicates that the decimal number phrase
         * is optional. Leading zeros are not allowed.
         *
         ...
         *
         * @param p the Parser object this UDT is attached to.
         * @param optional if true, the number is optional and
         *  <code>min = 0</code>, if false <code>min = 1</code>. For both,
         * <code>max = infinite</code> and leading zeros are not allowed.
         */
         ...
         *
         * @param p the Parser object this UDT is attached to.
         * @param minDigits minimum number of digits allowed (must be
         * <code>&gt;= 0</code>)
         * @param maxDigits maximum number of digits allowed (must be
         * <code>&gt;= minDigits</code>)
         * @param leadingZeros if true, leading zeros <i>are</i> allowed, if
         * false, leading zeros are <i>not</i> allowed.
         */
        ...
        public int callback(int offset) {
            ...
            int digits = 0;
            if (noLeadingZeros) {
                if (i < inlen && callbackData.inputString[i] == '0') {
                    return len;
                }
            }
     ...
     * (<code>32-126</code>) between beginning and ending quote characters, not
     * including the quote character. "Escaped" character pairs, such as
    ...
    public static class QuotedString extends Parser.UdtCallback {
        ...
        public int callback(int offset) {
            ...
            if (i < callbackData.inputString.length && callbackData.inputString[i] == quote) {
                ...
                for (; i < callbackData.inputString.length; i++) {
                    char c = callbackData.inputString[i];
                    if (((c >= PRINTING_BEG && c <= PRINTING_END) || c == HTAB) && c != quote) {
                        len++;
                    } else {
                        break;
                    }
    ...
    /**
     * UDT for the single-quoted string. The string begins and ends with the
     * single quote <code>' (x27)</code>.
     */
    ...
    /**
     * UDT for the double-quoted string. The string begins and ends with the
     * double quote <code>" (x22)</code>.
     */
     ...
     * <p>
     * This is a string of at least <code>min</code> and not more than
     * <code>max</code> hexidecimal digits, where
     * <code>0 &lt;= min &lt;= max</code>. A hexidecimal digit is
     * <code>0-9</code> or <code>A-F</code> (case insensitive).
     */
    public static class HexNum extends Parser.UdtCallback {
         ...
         * <p>
         * The default values are <code>min = 0, max = infinite</code>. Note
         * that <code>min = 0</code>, indicates that the hexidecimal number
         * phrase is optional.
         *
         * @param p the Parser object this UDT is attached to.
         */
         ...
         * @param p the Parser object this UDT is attached to.
         * @param optional if true, the number is optional and
         *  <code>min = 0</code>, if false <code>min = 1</code>. In either case
         * <code>max = infinite</code>.
         */
    ...
    /**
     * UDT for a "forgiving" line end. Forgiving means that anything resembling
     * either of the two most frequent standards will be accepted. Any of LF
     * (<code>x0A</code>), CRLF (<code>x0Dx0A</code>)or CR (<code>x0D</code>)
    ...
    public static class LFLineEnd extends LineEnd {
        ...
        /**
         * Default constructor. The line end is the single newline character,
         * x0A.
         *
         * @param parser the Parser object this UDT is attached to.
         */
    ...
    public static class CRLFLineEnd extends LineEnd {
        ...
        /**
         * Default constructor. The line end is the double character,
         * <code>x0Dx0A</code>.
         *
         * @param parser the Parser object this UDT is attached to.
         */
    ...
    /**
     * UDT for line end characters. Several conventions are commonly used for
     * line end characters in different common environments. Flexible options
     * are available.
     */
    public static class LineEnd extends Parser.UdtCallback {
        ...
        /**
         * Default constructor. The "forgiving" line end. Forgiving means that
         * anything resembling either of the two most frequent standards will be
         * accepted. Any of LF (<code>x0A</code>), CRLF (<code>x0Dx0A</code>)or
         * CR (<code>x0D</code>) are accepted as line ends.
         *
         * @param p the Parser object this UDT is attached to.
         */
        ...
        public int callback(int offset) {
            ...
            if (i < inlen) {
                if (charCount == 1) {
                    ...
                    if (ii < inlen) {
                        if (callbackData.inputString[i] == end1
                                && callbackData.inputString[ii] == end2) {
                            len = 2;
                        }
                    }
                } else {
                    if (callbackData.inputString[i] == end1) {
                        ...
                        len = 1;
                        i++;
                        if (i < inlen && callbackData.inputString[i] == end1) {
                            len++;
                        }
                    }
                }
    ...
    public static class WhiteSpace extends Parser.UdtCallback {
        ...
        /**
         * Default constructor. White space is an optional string of space
         * (<code>x20</code>) or horizontal tab (<code>x09</code>) characters.
         *
         * @param parser the Parser object this UDT is attached to.
         */
        ...
        /**
         * General constructor. White space always includes the standard white
         * space characters space(<code>x20</code>) or horizontal
         * tab(<code>x09</code>). Optionally white space additionally may
         * include line comments and/or folding white space. Folding white
         * space, found in many Internet standards, is a line end followed by
         * one standard white space character.
         *
         * @param parser the Parser object this UDT is attached to.
         * @param optional if true, the white space is optional, if false at
         * least one white space character or entity is required.
         * @param comment if null, comments are <i>not</i> considered white
         * space, otherwise comment is a Comment class object or any Comment
         * extension object.
         * @param foldingLineEnd if null, folding white space is not allowed.
         * Otherwise folding white space is allowed and foldingLineEnd is a
         * LineEnd class object or any LineEnd extension object.
         */
