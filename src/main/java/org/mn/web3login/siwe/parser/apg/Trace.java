 ...
 * journey through the syntax tree. It will generate a detailed information
 * record for every operator node the Parser visits (hits). Separate records are
 * generated for the pre-branch node hit (just prior to visiting the branch
 * below) and the post-branch node hit (just after visiting the branch below.)
 * <p>
 * Methods are available for controlling which nodes to create records for and
 * for displaying the records after the parse has completed. The
 * enabling/disabling of operator nodes is incremental. A few examples will
 * illustrate.
 * <p>
 * Suppose you wanted to display all operator nodes except the UDT operator
 * nodes. This could be accomplished with<br>
 * <code>
 ...
 * </code>
 * <p>
 * Or suppose you wanted to display all operator nodes <i>except for</i> rules
 * named "alpha", with grammar, id=3 and "beta", with grammar id=5<br>
 * <code>
 * enableAllNodes(true);<br>
 ...
 * </code>
 * <p>
 * Optionally, the trace records may be generated as an XML file. The user can
 * then use his/her favorite XML parser to peruse the trace.
 */
public class Trace {
    // public API

    /**
     * Resets the set of operator nodes to trace to the default set. The default
     ...
     *
     * @param enable if <code>true</code> all operator nodes will be traced, if
     * <code>false</code> none will be trace.
     */
     ...
     *
     * @param enable if <code>true</code> all rule name operators will be
     * traced, if <code>false</code> no rule name operators will be trace.
     */
    ...
    /**
     * Enables or disables the tracing of all terminal operators. Terminal nodes
     * are literal string operators (<code>TLS</code>), binary string operators
     * (<code>TBS</code>) and character range operators (<code>TRG</code>)
     *
     ...
     * concatenation operators (<code>CAT</code>), repetition operators
     * (<code>REP</code>), <code>"and"</code> syntactic predicate operators
     * (<code>AND</code>) and <code>"not"</code> syntactic predicate operators
     * (<code>NOT</code>)
     *
     * @param enable if <code>true</code> all non-terminal operators will be
     * traced, if <code>false</code> no non-terminal operators will be traced.
     */
     ...
     *
     * @param enable if <code>true</code> the named operator nodes will be
     * traced, if <code>false</code> the named operators will not be traced.
     * @param name name of the operator type to enable or disable<br>
     * <code>"alt"</code> enable/disable alternation (<code>ALT</code>)
     * operators<br>
     * <code>"cat"</code> enable/disable concatenation (<code>CAT</code>)
     * operators<br>
     * <code>"rep"</code> enable/disable repetition (<code>REP</code>)
     * operators<br>
     * <code>"and"</code> enable/disable "and" syntactic predicate
     * (<code>AND</code>) operators<br>
     * <code>"not"</code> enable/disable "not" syntactic predicate
     * (<code>NOT</code>) operators<br>
     * @throws Exception thrown if operator type name is unrecognized (none of
     * the above)
     */
    public void enableNode(boolean enable, String name) throws Exception {
        if (name.equalsIgnoreCase("alt")) {
            ...
            cat = enable;
        } else if (name.equalsIgnoreCase("rep")) {
            rep = enable;
     ...
     *
     * @param enable if <code>true</code> the named rule name operators will be
     * traced, if <code>false</code> the named rule name operators will not be
     * traced.
     ...
     *
     * @param enable if <code>true</code> the named UDT operators will be
     * traced, if <code>false</code> the named UDT operators will not be traced.
     ...
     *
     * @param enabled if <code>true</code>, XML format will be enabled, if
     * <code>false</code> the native APG format will be used.
     */
    ...
    /**
     * Sets the output device to record the trace records on. They are not
     * recorded in internal RAM memory.
     * <p>
     ...
     * </pre>
     *
     * @param out the output device. The default device is
    ...
    Trace(Parser parser, Grammar grammar) {
        ...
        suffixMaxChars = 128;
        if (ruleCount > 0) {
            rnmList = new boolean[ruleCount];
        } else {
            rnmList = null;
        }
        if (udtCount > 0) {
            udtList = new boolean[udtCount];
        } else {
            udtList = null;
        }
        enableDefaultNodes();
    ...
    void traceUp(char[] string, int offset, int length, boolean match, Opcode opcode) throws Exception {
        if (isNodeEnabled(opcode)) {
            if (enableXml) {
                ...
                out.print(opcode.toString());
                if (match) {
                    out.print(Utilities.charArrayToString(string, offset, length, suffixMaxChars));
                }
                out.println("");
            }
        }
    ...
    private String[] opcodeXMLAttrs(int tag, Opcode opcode) throws Exception {
        ...
        switch (opcode.type) {
            case ALT:
                ...
                ret[3] = "ALT";
                ret[4] = "children";
                ret[5] = String.format("%d", ((Opcode.Alt) opcode).children.length);
                break;
            case CAT:
                ret = new String[6];
                ret[2] = type;
                ret[3] = "CAT";
                ret[4] = "children";
                ret[5] = String.format("%d", ((Opcode.Cat) opcode).children.length);
                break;
            case AND:
                ret = new String[4];
                ret[2] = type;
            ...
            case REP:
                ...
                ret[3] = "REP";
                ret[4] = "min";
                ret[5] = String.format("%d", (int) ((Opcode.Rep) opcode).min);
                ret[6] = "max";
                ret[7] = String.format("%d", (int) ((Opcode.Rep) opcode).max);
                break;
            case RNM:
                ret = new String[8];
                ret[2] = type;
                ret[3] = "RNM";
                ret[4] = "id";
                ret[5] = String.format("%d", ((Opcode.Rnm) opcode).id);
                ret[6] = "name";
                ret[7] = ((Opcode.Rnm) opcode).name;
                break;
            case UDT:
                ret = new String[8];
                ret[2] = type;
                ret[3] = "UDT";
                ret[4] = "id";
                ret[5] = String.format("%d", ((Opcode.Udt) opcode).id);
                ret[6] = "name";
                ret[7] = ((Opcode.Udt) opcode).name;
                break;
            case TBS:
                ...
                ret[3] = "TBS";
                ret[4] = "length";
                ret[5] = String.format("%d", string.length);
                ret[6] = "name";
                ret[7] = Utilities.charArrayToXml(string, 0, -1);
                break;
            case TLS:
            ...
            case TRG:
                ...
                ret[3] = "TRG";
                ret[4] = "min";
                ret[5] = String.format("%d", (int) ((Opcode.Trg) opcode).min);
                ret[6] = "max";
                ret[7] = String.format("%d", (int) ((Opcode.Trg) opcode).max);
                break;
            default:
                throw new Exception("Trace: opcodeXMLAttrs: unknown opcode type");
        }
        ret[0] = "tag";
        ret[1] = String.format("%d", tag);
        return ret;
    }
